...
view: medical_expense_top_and_bottom {
  derived_table: {
    sql: WITH TOP_CTE AS (
                SELECT UNIQUE_ID,YEAR,AMT
              FROM (
                SELECT UNIQUE_ID, SUM(TOTAL_EMPLOYER_PAID_AMT) AS AMT,YEAR(DIAGNOSIS_DATE) AS YEAR,
  ...
  dimension: medical_spend_top_and_bottom_1_percent {
    type: string
